...
void MainWindow::onEndInteraction(vtkObject* caller, long unsigned int eventId, void* clientData, void* callData)
{
    ...
    if (renderer)
    {
        ...
        if (camera)
        {
            ...
            // Issue the rotation commands to the VR thread
            vrThread->issueCommand(VRRenderThread::ROTATE_X, orientation[0]);
            vrThread->issueCommand(VRRenderThread::ROTATE_Y, orientation[1]);
            vrThread->issueCommand(VRRenderThread::ROTATE_Z, orientation[2]);
        }
    }
}
